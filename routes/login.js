...
exports.post = function (req, res) {
    if (req.body.username == "" || req.body.password == ""
        ...
        res.render('login', {login_error: 'You did not fill out all the fields'});
        return;
    }
    console.log(req.session);
    console.log(req);
    ...
    client.get("user:" + username + ":password", function (err, reply) {
        if (reply == "" || reply == null) {
            res.render('login', {login_error: 'Username or password is incorrect.'});
            return;
        }
        bcrypt.compare(password, reply, function (err, valid) {
            ...
            client.get("user:" + username + ":permissions", function (err, reply) {
                if (S(reply).contains(";")) {
                    ...
                    client.sadd("online", username, function (err, reply) {
                        //We do nothing with the information. It's just "OK" anyways.
                    });
                    res.redirect('/box');
